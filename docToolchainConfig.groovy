...
inputFiles = [
    ...
    [file: '02-arc42/arc42.adoc', formats: ['html','pdf1','docbook']],
    [file: '04-runbook/00-runbook.adoc', formats: ['html','pdf2']],
    [file: 'Demo-Slides.adoc', formats: ['revealjs']],
	/** inputFiles **/
]
...
microsite.with {
    ...
    // the server name of the deployed site.
    // Example https://docs-as-code.gitpages.tech.rz.db.de
    host = 'https://docs-as-code.gitpages.tech.rz.db.de'
    // if not deployed to the context root, this is the fix part of the URL between the server and the first page
    // for gitlab or github pages, this is the name of the repository.
    // make sure that there is no `/` at the end.
    // Example /myrepository
    contextPath = ''
    // this one is needed for the sitemap.xml
    contextPathSiteMap = '/documentation'
    // this let's the .gitlab-ci.yml overwrite the contextPath for branch previews
    if (System.getenv('CONTEXT_PATH')) {
        contextPath = System.getenv('CONTEXT_PATH')
    }
    
    // the port on which the server started by previewSite should be running.
    // Eample 8089
    previewPort = 8042
    // the title of the microsite, displayed in the upper left corner
    title = 'Demo'
    // the location of the landing page
    landingPage = '../doc/landingpage.gsp'
    siteFolder = '../site'

    // the menu of the microsite. A map of [code:'title'] entries to specify the order and titles of the entries.
    // the codes are the :jbake-menu: entries from the .adoc files
    menu = [onboarding: 'Onboarding', arc42:'Architektur', interfaces:'Interfaces', runbook:'Runbook', guidelines: 'Guidelines', About: 'About', test:'-', blog:'-', Markdown:'-', slides: '-']
    // html fragment of the search input
    search = """
        <form class="form-inline my-2 my-lg-0" action="https://search.noncd.rz.db.de/login">
                <input type="hidden" name="site" value="${host-"https://"}" />
                <input style="width: 240px;" type="text" name="q" class="form-control mr-sm-2" placeholder="Search">
        </form>
    """
    // html fragment of the search input for the local lunr based search
    /**
    search = """
        <form class="form-inline my-2 my-lg-0" onSubmit="this.action=toRoot+'search.html';return true;">
                <input style="width: 240px;" type="text" name="q" class="form-control mr-sm-2" placeholder="Search">
        </form>
    """
    **/
    ...
    footerText = '<small class="text-white">built with <a href="https://doctoolchain.org">docToolchain</a> and <a href="https://jbake.org">jBake</a>,<br />styling supported by <a href="https://db-ui.github.io/">DB UI Core</a></small>'

    // if set, the microsite master will use two pixel images to create some user statistics
    // via https://db.de/urlshortener
    // remove if unwanted
    userStats = [
            counter1: 'n0ziog',
            counter2: 'vivzc2',
    ]
    gitRepoUrl = 'https://git.tech.rz.db.de/docs-as-code/templates/micrositemaster-ng/-/tree/master/src/docs'
    issueUrl = 'https://git.tech.rz.db.de/docs-as-code/templates/micrositemaster-ng/-/issues/new'
